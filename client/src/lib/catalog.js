// Deterministic catalog generator for a large, realistic supermarket inventory.
// Produces stable SKUs/prices/stock so the demo store looks fully stocked and
// the same data appears on every load. Mirrored by server/src/utils/catalog.js.
...
export const toNaira = (usd) => Math.round((usd * 1500) / 50) * 50;
...
// Tiny deterministic PRNG so generated data is stable across reloads.
function pseudo(n) {
...
export function buildExtraProducts(count = 100) {
  // Flatten base items, then extend with size variants until we have enough.
  const base = [];
  ...
  return pool.slice(0, count).map((p, i) => {
    ...
    const lowStockThreshold = 8 + Math.floor(r2 * 12); // 8..19
    // ~18% of items are intentionally low/out of stock to exercise alerts.
